...
if (version_compare(PHP_VERSION, '5.3.7', '<')) {
    ...
    require_once("../libraries/password_compatibility_library.php");
		...
		if (empty($_POST['name2'])){
            ...
            $errors[] = "Nombre de puerto vacío";
        }  elseif (strlen($_POST['user_name2']) > 64 || strlen($_POST['user_name2']) < 2) {
            $errors[] = "Nombre de puerto no puede ser inferior a 2 o más de 64 caracteres";
        } elseif (!preg_match('/^[a-z\d]{2,64}$/i', $_POST['user_name2'])) {
            $errors[] = "Nombre de puerto no encaja en el esquema de nombre: Sólo aZ y los números están permitidos , de 2 a 64 caracteres";
        } elseif (empty($_POST['user_email2'])) {
            $errors[] = "El nombre de la inspectoria no puede estar vacío";
          ...
          )
         {
			...
			require_once ("../config/conexion.php");//Contiene funcion que conecta a la base de datos
                ...
                $firstname = mysqli_real_escape_string($con,(strip_tags($_POST["name2"],ENT_QUOTES)));
				...
				$user_id=intval($_POST['mod_id']);
                    ...
                    $sql = "UPDATE pto_base SET firstname='".$firstname."', lastname='".$lastname."', user_name='".$user_name."', user_email='".$user_email."'
                            WHERE user_id='".$user_id."';";
                    $query_update = mysqli_query($con,$sql);

                    // if user has been added successfully
                    if ($query_update) {
                        $messages[] = "Ha sido modificada con éxito.";
                    } else {
