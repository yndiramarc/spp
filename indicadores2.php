...
<body>
  ...
  <table class="table table-striped table-bordered table-hover" align="center">
                            ...
                            <th class='text-center'>N° Desembarques</th>
                            <th class='text-center'>Porcentaje Desembarques</th>
                            <th class='text-center'>Total Kg</th>
                            <th class='text-center'>Porcentaje Kg</th>
...
group by year(fecha_factura)';  
                    ...
                    {
                        $a++;
                         echo "<tr><td class='text-center'>".$categoria[$j];
                        echo "<td class='text-center'>".$datos[$j]."</td>";
                    //    echo "<td>".$kg1[$j]."</td>";
                    
            echo "<td class='text-center'>".number_format((($datos[$j]/$desembarques1)*100), 1, ',', '')."%";
            echo "<td class='text-center'>".number_format(($kg1[$j]), 2, ',', '.');

          
            $por[$j]= round( ($datos[$j]/$cantdFact)*100, 2);
            echo "<td class='text-center'>".number_format((($kg1[$j]/$kg)*100), 2, ',', '')."%";
    ...
    <a href="reportes.php" class="btn btn-default btn-block">Grafico</a>
  ...
  <table class="table table-striped table-bordered table-hover">
                    <thead>
                        ...
                        <th class='text-center'>Año</th>
                         <th class='text-center'>Total Kg</th>
                            <th class='text-center'>Cañera</th>
                            <th class='text-center'>Palangre</th>
                            <th class='text-center'>Polivalente</th>
                            <th class='text-center'>Cerquera</th>
                            <th class='text-center'>Costa Afuera</th>
...
and products.id_producto = detalle_factura.id_producto
 group by year order by year(facturas.fecha_factura)';  
                    ...
                    {
                        $a++;
                        echo "<tr class='text-center'><td>".$categoria[$j]."</td>";
                    ...
                    ?>
                              ...
                              <a href="../SGP2/reporte_artes.php" class="btn btn-default btn-block">Grafico</a>
 ...
 <div>
                <div >
                    <div class="panel panel-default">
                        <div class="panel-heading">
                            <i class="fa fa-bar-chart-o fa-fw"></i> CPUE
                            <div class="pull-right">
                                <div class="btn-group">
  ...
  <table class="table table-striped table-bordered table-hover">
           <thead>
                <tr>
                    <th class='text-center'>Año</th>
                    <th class='text-center'>Numero de Embarcaciones</th>
                     <th class='text-center'>Total Kg </th>
                      <th class='text-center'>CPUE</th>
 ...
 <?php
  ...
  $query2 = 'SELECT year(fecha_factura) as year, count(facturas.id_factura) as desembarques2 FROM facturas WHERE year(fecha_factura)<= year(curdate())
group by year(fecha_factura)';  
                    ...
                    {
                        $categoria[$c] = $row["year"];
                        $datos[$c] = $row["desembarques2"];   
                        $desembarques1 = $desembarques1 + $row["desembarques2"];
                         $sum1=  $sum1+ $row["desembarques2"];
                        $c++;
                    }
                    for ($j=0;$j<=$c-1;$j++)
                    {
 ...
 <?php
            ...
            $query = 'SELECT  year(facturas.fecha_factura) AS year,  count( facturas.id_cliente)  AS numeroEmb, 
            SUM( detalle_factura.cantidad ) AS totalano,
...
FROM detalle_factura, facturas, clientes
            ...
            while($row = mysqli_fetch_array($result, MYSQLI_ASSOC))
            {
              ...
              //  $datos[$c] = $row["numeroEmb"];   
                ...
                $totalPro[$c]= $row["totalano"];

                $total2[$c] =  $row["numeroEmb"];

                 $cpue[$c] =  $totalPro[$c] / $datos[$c];
   $sumcpue= $sumcpue + $cpue[$c];
            ...
            for ($j=0;$j<=$c-1;$j++)
            {
             ...
             echo "<td>".number_format(($totalPro[$j]), 2, ',', '.');
             echo "<td>".number_format(($cpue[$j]), 2, ',', '.');
             echo "<td>".number_format((($cpue[$j]/$sumcpue)*100), 2, ',', '')."%"."</tr>";
        ...
        /*        if ($j==0)  
                {
                    echo "</td><td rowspan=".$c.">".$sum1."</td></tr>";
                }*/
            }
            mysqli_close($con);       
            ?>
