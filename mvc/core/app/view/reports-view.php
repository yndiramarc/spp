...
<option value="">PACIENTE</option>
  <?php foreach($pacients as $p):?>
    <option value="<?php echo $p->id; ?>" <?php if(isset($_GET["pacient_id"]) && $_GET["pacient_id"]==$p->id){ echo "selected"; } ?>><?php echo $p->id." - ".$p->nombre_cliente." ".$p->telefono_cliente; ?></option>
  <?php endforeach; ?>
</select>
		</div>
    </div>
...
//echo $sql;
		...
		if(count($users)>0){
			...
			foreach($users as $user){
				...
				<td><?php echo $pacient->nombre_cliente." ".$pacient->telefono_cliente; ?></td>
				<td><?php echo $medic->id_factura." ".$medic->id_factura; ?></td>
				<td><?php echo $user->date_at." ".$user->time_at; ?></td>
				<td><?php echo $user->getStatus()->estado_factura; ?></td>
				<td><?php echo $user->getPayment()->condiciones; ?></td>
				<td>$ <?php echo number_format($user->total_venta,2,".",",");?></td>
				</tr>
				<?php
				$total += $user->price;
